...
- **项目指令文件（AGENTS.md）。** 在工作区根目录放一份 `AGENTS.md`（也识别 `CLAUDE.md` / `EVA.md`，按此优先级取第一个存在的），每回合自动注入系统提示：项目约定、常用命令、代码规范写一次，主智能体和委派的子智能体都会遵守（超过 32K 字符自动截断）。
- **上下文自动压缩。** 长会话估算 token 超过窗口约七成时，较早的消息自动总结成一份持久摘要，模型上下文变为「摘要 + 近期消息」，任务不因窗口耗尽而"失忆"；聊天历史与回放保持完整，压缩失败时回退硬截断兜底。
- **Plan 模式。** 权限模式切到「计划」后，智能体先做只读调研——所有变更类工具与命令被运行时门控拦下并引导它先出计划；调研完用 ExitPlanMode 呈交 markdown 计划，你在卡片上「批准并开始」（按自动编辑继续）、「批准 · 完全访问」（本回合不再询问）或驳回附反馈让它修订重交。获批后同一回合继续执行，已派生的子智能体同步解锁。
- **命令级放行规则。** 审批从"工具级"细化到"命令级"：内置只读命令表（git status / diff / log、ls、cat、grep、Get-ChildItem 等）命中的命令免卡直接执行；你在命令审批卡上选「始终允许此类命令」即可按命令开头一键建规则（全局或项目级），也可在 **设置 → 工具** 手动维护。安全边界：复合命令逐段检查（`git status && rm -rf /` 整体弹卡）、含重定向或命令替换（`>` `$()` 反引号）的命令永不放行、token 边界匹配（"pnpm test" 不匹配 "pnpm test2"）、危险命令（rm、git push 等）拒绝建规则。
- **内置终端。** 右侧面板新增「终端」页签：完整交互式 shell（Windows PowerShell / macOS·Linux 跟随 $SHELL），cwd 跟随当前项目，支持多会话、自适应尺寸与 256 色输出。终端由你直接操作，不经过智能体审批链；输出保留 256KB 环形缓冲，切页签或刷新回来历史不丢，进程退出可一键重开。开发模式基于 node-pty 实时加载；打包版 sidecar 暂未随包分发原生模块，终端页签会显示不可用提示（其余功能不受影响）。
- **可配置的默认工作区。** 首次启动会引导你选择一次默认工作区（智能体读写文件、生成物与附件的根目录）——浏览选一个常用目录，或直接采纳默认位置；之后在 **设置 → 常规** 随时更改，文件面板、终端 cwd 与无项目会话即时切换。
- **MCP 服务器扩展。** 在 **设置 → MCP** 添加 stdio（本地进程）或 HTTP（Streamable HTTP / SSE）服务器，支持全局级 / 项目级作用域、连接测试与工具清点；启用后其工具以 `mcp__服务器__工具` 进入模型上下文，权限与审批策略同内置工具一致。
- **项目与会话管理。** 多项目分组的侧边栏，会话与项目支持置顶、归档、重命名。
- **知识库（llm-wiki 模式）。** 对话回合完成后自动总结成持久 wiki：实体、概念、查询页面互相以 `[[双链]]` 引用，项目对话沉淀到对应项目的专属知识库。支持手动上传文档解析（md / txt / html / pdf / docx / xlsx / pptx，提取文本后总结为来源页并抽取实体概念）。左侧目录树点击预览、支持编辑，一键导出 Obsidian 兼容 vault；知识图谱用力导向布局展示页面关联与社区聚类；也可从历史会话一键批量构建。
